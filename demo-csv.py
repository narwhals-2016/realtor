...
demo_csv = pd.read_csv('maspeth-demo.csv', skiprows=[1,2,3], converters={'Unnamed: 1': lambda x: x.replace(',','')})

indexed = demo_csv.set_index('2009-2013 ACS Demographic Profile')

# the following return series, which we can .loc[n] and convert to int
under_nineteen = sum([
